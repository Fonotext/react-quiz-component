...
function Quiz({
  ...
  const nrOfQuestions = quiz.nrOfQuestions && Number(quiz.nrOfQuestions) < quiz.questions.length
    ? Number(quiz.nrOfQuestions)
    : quiz.questions.length;

  // Shuffle answers funtion here
  const shuffleAnswerSequence = (oldQuestions: QuizQuestion[] = []) => {
  ...
  const shuffleQuestions = useCallback((q: QuizQuestion[]) => {
  ...
  const validateQuiz = (q: typeof quiz) => {
    if (!q) {
      console.error('Quiz object is required.');
      return false;
    }

    if ((timer && typeof timer !== 'number') || (timer && timer < 1)) {
